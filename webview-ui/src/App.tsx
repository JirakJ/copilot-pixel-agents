...
function App() {
  ...
  const handleWhatsNewDismiss = useCallback(() => {
    bridge.send({ type: 'setLastSeenVersion', version: currentMajorMinor });
  }, [currentMajorMinor]);
  ...
  const handleToggleAlwaysShowOverlay = useCallback(() => {
    setAlwaysShowOverlay((prev) => {
      const newVal = !prev;
      bridge.send({ type: 'setAlwaysShowLabels', enabled: newVal });
      return newVal;
    });
  }, []);

  const handleSelectAgent = useCallback((id: number) => {
    bridge.send({ type: 'focusAgent', id });
  }, []);
  ...
  const handleCloseAgent = useCallback((id: number) => {
    bridge.send({ type: 'closeAgent', id });
  }, []);
  ...
  const handleClick = useCallback((agentId: number) => {
    ...
    bridge.send({ type: 'focusAgent', id: focusId });
  }, []);
    ...
    >
      ...
      <BottomToolbar
        ...
        onToggleWatchAllSessions={() => {
          ...
          bridge.send({ type: 'setWatchAllSessions', enabled: newVal });
        }}
      />
